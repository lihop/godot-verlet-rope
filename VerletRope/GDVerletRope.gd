	...
	for i in range(0, simulation_particles - 1):
		...
		if cam_dist_particle.length_squared() <= subdiv_lod_distance * subdiv_lod_distance:
			...
			if tangent_dots >= COS_5_DEG:
				# 0 subdivisions
				interval_data = RopeUtil.catmull_interpolate_in_step_one(p0, p1, p2, p3)
				rope_draw_subdivs = 1.0
			elif tangent_dots >= COS_15_DEG:
				# 2 subdivisions
				interval_data = RopeUtil.catmull_interpolate_in_step_halfs(p0, p1, p2, p3)
				rope_draw_subdivs = 0.5
			elif tangent_dots >= COS_30_DEG:
				# 3 subdivisions
				interval_data = RopeUtil.catmull_interpolate_in_step_thirds(p0, p1, p2, p3)
				rope_draw_subdivs = 0.333333
			else:
				# 4 subdivisions
				interval_data = RopeUtil.catmull_interpolate_in_step_fourths(p0, p1, p2, p3)
...
func _draw_catmull_curve() -> void:
	...
	for i in range(0, simulation_particles - 1):
		...
		while t <= 1.0:
			...
			var point2_data: PoolVector3Array = RopeUtil.catmull_interpolate(p0, p1, p2, p3, min(t + step, 1.0))
